    ...
    if target_chat:
        ...
        for hackathon in new_hackathons:
            try:
                ...
                logger.info(
                    f"Sent notification for hackathon '{hackathon.title}' to chat {target_chat}"
                )
            except Exception as e:
        ...
        try:
            ...
            for config in configs:
                ...
                platforms = (
                    config.subscribed_platforms.split(",")
                    if config.subscribed_platforms
                    else ["all"]
                )
                themes = (
                    config.subscribed_themes.split(",") if config.subscribed_themes else ["all"]
                )
                ...
                for hackathon in new_hackathons:
                    ...
                    if "all" not in themes:
                        hack_tags = (
                            [t.lower() for t in hackathon.tags.split(",")] if hackathon.tags else []
                        )
                        match = False
                        for theme in themes:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Sent notification for hackathon '{hackathon.title}' to chat {chat_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to send hackathon notification to chat {chat_id}: {e}"
                        )
        finally:
            db.close()
        ...
        for user_id, hacks in user_notifications.items():
            try:
                for hack in hacks:
                    ...
                    alert_text = (
                        "🔔 <b>New Hackathon Alert!</b> (Matches your subscription)\n\n" + text
                    )
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "⭐ Star on GitHub", url="https://github.com/Spartan-71/Telegram-Hackathon-Bot"
            )
        ],
    ]
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "🐛 Report Bug", url="https://github.com/Spartan-71/Telegram-Hackathon-Bot/issues"
            )
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        about_text, parse_mode=ParseMode.HTML, reply_markup=reply_markup
    )
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔍 Searching for hackathons matching '<b>{keyword}</b>'...", parse_mode=ParseMode.HTML
    )
    ...
    if not results:
        await update.message.reply_text(
            f"❌ No hackathons found for <b>{keyword}</b>", parse_mode=ParseMode.HTML
        )
        return

    await update.message.reply_text(
        f"🔍 Found <b>{len(results)}</b> hackathon(s) for <b>{keyword}</b>:",
        parse_mode=ParseMode.HTML,
    )
    await send_hackathon_notifications(
        context.application, results, target_chat=update.effective_chat.id
    )
...
async def platform_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔍 Fetching hackathons from <b>{platform_name}</b>...", parse_mode=ParseMode.HTML
    )
    ...
    if not results:
        await update.message.reply_text(
            f"❌ No hackathons found for platform <b>{platform_name}</b>", parse_mode=ParseMode.HTML
        )
        return

    await update.message.reply_text(
        f"🔍 Found <b>{len(results)}</b> hackathon(s) from <b>{platform_name}</b>:",
        parse_mode=ParseMode.HTML,
    )
    await send_hackathon_notifications(
        context.application, results, target_chat=update.effective_chat.id
...
async def upcoming_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📅 Fetching hackathons starting in the next <b>{days}</b> days...",
        parse_mode=ParseMode.HTML,
    )
    ...
    if not results:
        await update.message.reply_text(
            f"❌ No upcoming hackathons found in the next <b>{days}</b> days.",
            parse_mode=ParseMode.HTML,
        )
        return

    await update.message.reply_text(
        f"📅 Found <b>{len(results)}</b> upcoming hackathon(s) in the next <b>{days}</b> days:",
        parse_mode=ParseMode.HTML,
    )
    await send_hackathon_notifications(
        context.application, results, target_chat=update.effective_chat.id
...
async def subscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_new:
            await update.message.reply_text(
                f"✅ You have successfully subscribed to <b>{theme}</b> updates!",
                parse_mode=ParseMode.HTML,
            )
        else:
            await update.message.reply_text(
                f"ℹ️ You are already subscribed to <b>{theme}</b>.", parse_mode=ParseMode.HTML
            )
    except Exception as e:
...
async def unsubscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if removed:
            await update.message.reply_text(
                f"✅ You have successfully unsubscribed from <b>{theme}</b> updates.",
                parse_mode=ParseMode.HTML,
            )
        else:
            await update.message.reply_text(
                f"ℹ️ You were not subscribed to <b>{theme}</b>.", parse_mode=ParseMode.HTML
            )
    except Exception as e:
...
async def setup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        setup_text, parse_mode=ParseMode.HTML, reply_markup=reply_markup
    )
    ...
    if data == "help":
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Devfolio"
                    if "devfolio" in context.chat_data.get("setup_platforms", [])
                    else "Devfolio",
            ...
            [
                InlineKeyboardButton(
                    "✅ Devpost"
                    if "devpost" in context.chat_data.get("setup_platforms", [])
                    else "Devpost",
            ...
            [
                InlineKeyboardButton(
                    "✅ Unstop"
                    if "unstop" in context.chat_data.get("setup_platforms", [])
                    else "Unstop",
            ...
            [
                InlineKeyboardButton(
                    "✅ DoraHacks"
                    if "dorahacks" in context.chat_data.get("setup_platforms", [])
                    else "DoraHacks",
            ...
            [
                InlineKeyboardButton(
                    "✅ Hack2Skill"
                    if "hack2skill" in context.chat_data.get("setup_platforms", [])
                    else "Hack2Skill",
            ...
            [
                InlineKeyboardButton(
                    "✅ Kaggle"
                    if "kaggle" in context.chat_data.get("setup_platforms", [])
                    else "Kaggle",
            ...
            [
                InlineKeyboardButton(
                    "✅ MLH" if "mlh" in context.chat_data.get("setup_platforms", []) else "MLH",
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Devfolio" if "devfolio" in platforms else "Devfolio",
                    callback_data="toggle_platform_devfolio",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Devpost" if "devpost" in platforms else "Devpost",
                    callback_data="toggle_platform_devpost",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Unstop" if "unstop" in platforms else "Unstop",
                    callback_data="toggle_platform_unstop",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ DoraHacks" if "dorahacks" in platforms else "DoraHacks",
                    callback_data="toggle_platform_dorahacks",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Hack2Skill" if "hack2skill" in platforms else "Hack2Skill",
                    callback_data="toggle_platform_hack2skill",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Kaggle" if "kaggle" in platforms else "Kaggle",
                    callback_data="toggle_platform_kaggle",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ MLH" if "mlh" in platforms else "MLH", callback_data="toggle_platform_mlh"
                )
            ],
            [InlineKeyboardButton("« Back", callback_data="setup_back")],
        ]
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ AI/ML" if "ai" in context.chat_data.get("setup_themes", []) else "AI/ML",
            ...
            [
                InlineKeyboardButton(
                    "✅ Blockchain/Web3"
                    if "blockchain" in context.chat_data.get("setup_themes", [])
            ...
            [
                InlineKeyboardButton(
                    "✅ Web Development"
                    if "web" in context.chat_data.get("setup_themes", [])
            ...
            [
                InlineKeyboardButton(
                    "✅ Mobile App"
                    if "mobile" in context.chat_data.get("setup_themes", [])
                    else "Mobile App",
            ...
            [
                InlineKeyboardButton(
                    "✅ Data Science"
                    if "data" in context.chat_data.get("setup_themes", [])
                    else "Data Science",
            ...
            [
                InlineKeyboardButton(
                    "✅ IoT" if "iot" in context.chat_data.get("setup_themes", []) else "IoT",
                    callback_data="toggle_theme_iot",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Cloud" if "cloud" in context.chat_data.get("setup_themes", []) else "Cloud",
            ...
            [
                InlineKeyboardButton(
                    "✅ Cybersecurity"
                    if "security" in context.chat_data.get("setup_themes", [])
                    else "Cybersecurity",
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ AI/ML" if "ai" in themes else "AI/ML", callback_data="toggle_theme_ai"
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Blockchain/Web3" if "blockchain" in themes else "Blockchain/Web3",
                    callback_data="toggle_theme_blockchain",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Web Development" if "web" in themes else "Web Development",
                    callback_data="toggle_theme_web",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Mobile App" if "mobile" in themes else "Mobile App",
                    callback_data="toggle_theme_mobile",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Data Science" if "data" in themes else "Data Science",
                    callback_data="toggle_theme_data",
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ IoT" if "iot" in themes else "IoT", callback_data="toggle_theme_iot"
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Cloud" if "cloud" in themes else "Cloud", callback_data="toggle_theme_cloud"
                )
            ],
            [
                InlineKeyboardButton(
                    "✅ Cybersecurity" if "security" in themes else "Cybersecurity",
                    callback_data="toggle_theme_security",
                )
            ],
            [InlineKeyboardButton("« Back", callback_data="setup_back")],
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "⭐ Star on GitHub",
                        url="https://github.com/Spartan-71/Telegram-Hackathon-Bot",
                    )
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def post_init(application: Application) -> None:
    ...
    scheduler.add_job(check_and_notify_hackathons, "interval", hours=12, args=[application])
    scheduler.start()
